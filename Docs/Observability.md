...
Socket response delivery now has a ten-second total write deadline, including the terminating newline. Serialized response bodies over 4 MiB are rejected before writing. Failed delivery closes the connection and records `rpc.response.failed` with error kind and serialized byte count; it currently lacks the originating request trace ID. This caps delivery time and wire size, not allocations already made while handlers construct/serialize response values.
