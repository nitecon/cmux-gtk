...
Durable review comments are stored by canonical Git repository and exposed without a running application through `cmux comments list/add/delete/consume`. Pending entries carry stable UUIDs, old/new line ranges, captured line text, a message and a ready-to-copy agent submission. Git-backed diff generation snapshots pending comments into the private viewer, which renders them at the range's ending line in either layout. Consumed comments remain available through `list --all`, while delete removes them. Per-repository locking prevents lost updates across agents; storage is owner-only, atomically replaced, capped at 512 comments and 1 MiB, and pruned globally to 128 repositories or 64 MiB.
