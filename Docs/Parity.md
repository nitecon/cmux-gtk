...
| February baseline / June | Sidebar metadata and project/diff views | Branch/PR/MR, status/progress/markdown metadata, description and latest notification; automatic Git updates during foreground work; project/diff views and API with bounded worker I/O. | **Partial; cumulative diff verification pending.** Metadata, markdown, Git polling, ports and latest notification are implemented ([metadata](#agent-sidebar-metadata-checkpoint), [Git provenance](#git-tracking-and-provenance-checkpoint)). Patch/unstaged/staged/branch and per-agent-turn diff surfaces have bounded self-contained browser views and native placement; durable repository review comments now have agent CLI controls and line-anchored rendering ([diff surfaces](#agent-accessible-diff-surface-checkpoint)). The dedicated project view remains. |
...
This checkpoint remains incomplete parity evidence until Actions passes. The separate dedicated project view also remains.
