...
| May / pinned README | Agent hooks | Native IDs, environment-aware resume and setup integration for all 13 advertised agents; custom bindings, stale protection, notification routing, lifecycle traces and restore benchmark. | **Implemented; Actions pending for newest providers.** Claude/Codex/shared JSON/OpenCode/Cursor/Pi/Amp/Rovo checkpoints below cover the complete pinned list. The latest cumulative run must execute every fixture before this row is verified. |
...
| February baseline / June | Sidebar metadata and project/diff views | Branch/PR/MR, status/progress/markdown metadata, description and latest notification; automatic Git updates during foreground work; project/diff views and API with bounded worker I/O. | **Partial.** Metadata, markdown, Git polling, ports and latest notification are implemented ([metadata](#agent-sidebar-metadata-checkpoint), [Git provenance](#git-tracking-and-provenance-checkpoint)); dedicated project and diff viewer surfaces remain. |
...
| July | Workspace/surface reorder shortcuts | Batch and keyboard reorder, pane/surface move and drag topology with correct ownership, focus and remote routing. | **Partial; same-workspace implementation awaiting Actions.** Workspace batch/keyboard moves are implemented ([workspace moves](#workspace-move-shortcuts-checkpoint)); stable tab reorder, cross-pane move and placeholder-free directional split now have a native boundary and fixture ([surface topology](#surface-move-and-drag-topology-checkpoint)). Cross-workspace route rebinding and direct pointer split-drop remain. |
...
Pi uses its native auto-discovered TypeScript extension under `${PI_CODING_AGENT_DIR:-~/.pi/agent}/extensions`, not a fabricated hooks table. Setup writes only an owner-marked `cmux-session.ts`, refuses an unowned file or symlink, and atomically syncs an idempotent generated module. Its bounded, detached five-second dispatch registers Pi's `session_start`, `agent_end`, and `session_shutdown` callbacks, forwarding only the native session identity, current directory and bounded completion reason through the inherited exact surface. The shared handler stores `pi --session SESSION_ID`, publishes completion attention and conditionally clears the matching checkpoint on shutdown. Actions imports the generated ESM without Pi dependencies, invokes all callbacks against live GTK and executes the exact saved provider command.
