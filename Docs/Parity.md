...
| May / current catalog | Agent hooks | Native IDs, environment-aware resume and setup integration for every advertised agent; custom bindings, stale protection, notification routing, lifecycle traces and restore benchmark. | **Implemented; cumulative Actions pending.** The provider checkpoints cover all 18 entries in the current upstream hook catalog plus Claude's wrapper-based integration, using their native JSON, YAML, TOML or extension formats. Runtime claims remain conditional on the latest complete Actions run. |
...
| February baseline / June | Sidebar metadata and project/diff views | Branch/PR/MR, status/progress/markdown metadata, description and latest notification; automatic Git updates during foreground work; project/diff views and API with bounded worker I/O. | **Implemented; cumulative verification pending.** Metadata, markdown, Git polling, ports and latest notification are implemented ([metadata](#agent-sidebar-metadata-checkpoint), [Git provenance](#git-tracking-and-provenance-checkpoint)). Patch/unstaged/staged/branch and per-agent-turn diff surfaces have bounded self-contained browser views and native placement; durable repository review comments have agent CLI controls and line-anchored rendering ([diff surfaces](#agent-accessible-diff-surface-checkpoint)). The Linux project inspector adapts upstream's Xcode-only Files/Targets/Build Settings/Schemes surface to bounded manifest discovery for Rust, Go, Node, Python, CMake, Make and cmux actions ([project inspector](#manifest-aware-linux-project-inspector-checkpoint)). |
...
| July | Workspace/surface reorder shortcuts | Batch and keyboard reorder, pane/surface move and drag topology with correct ownership, focus and remote routing. | **Linux adaptation implemented; cumulative Actions pending.** Workspace moves, stable tab reorder, cross-pane/cross-workspace transfer, browser route rebinding, pane-center/edge and sidebar pointer drops, and placeholder-free directional split share one bounded mutation path ([workspace moves](#workspace-move-shortcuts-checkpoint), [surface topology](#surface-move-and-drag-topology-checkpoint)). The deterministic fixture verifies that path and restored topology. The final terminal in an SSH workspace stays in place because its PTY transport belongs to that workspace; the command rejects the move instead of terminating or silently converting the remote session. Other remote surfaces move normally. |
| July–August | Mosh / remote resume | Implement resilient remote transport and remote resume; explicitly verify replacement/EOF semantics. | **Implemented; cumulative Actions pending.** Mosh, Mosh-tmux, capability probing, fallback and persistence are implemented ([Mosh transport](#mosh-interactive-transport)). Signed direct-SSH resume input is queued exactly once after the restored stream opens; the real SSH fixture approves through GTK and verifies remote execution after restart. Mosh embeds approved bindings only in its remote command and direct-SSH fallback, with executable builder coverage. The real SSH fixture also proves EOF retains the surface with an explicit diagnostic policy and the next key closes only that ended surface. |
...
| Current README | Window/session fidelity | Audit multi-window controls and restore semantics; implement applicable behavior and native visual lifecycle tests. | **Linux single-window adaptation implemented and previously verified.** Geometry, workspace order, split topology, surfaces, browser locations, scrollback and active selection restore inside the GTK application window. Creating and routing multiple native windows is outside this milestone; see [current-main scope boundary](#current-main-scope-boundary). |
...
## Current-main scope boundary

This milestone implements the previously documented monthly gaps and the explicitly requested Linux workflows. The September 6 current-main audit also found newer upstream features that do not redefine this release:

- **Browser import:** upstream imports authentication data from many installed browsers. Linux cmux provides explicit named persistent browser profiles, including restoration, without reading another browser's cookie database. Cross-browser credential migration is a separate security-sensitive feature.
- **Remote image drop:** upstream can drag an image into an SSH terminal and upload it with `scp`. Linux remote terminals, startup commands and browser routing are covered here; a drag-and-upload convenience flow is separate from the requested remote workspace capability.
- **Multiple native windows:** this GTK build intentionally restores one application window with all workspaces. A multi-window ownership model is a separate product change, not a prerequisite for reliable workspace and terminal restoration.
- **Opt-in local tmux and agent hibernation:** this build restores bounded terminal history and reinitializes terminals, then resumes supported agents through reviewed native hooks. It does not claim arbitrary process checkpointing or silently add a terminal-multiplexer dependency.

These differences are explicit adaptations. They are not represented as implemented upstream-equivalent features.
...
Approved resume bindings for direct-SSH workspaces now produce one bounded, shell-quoted subshell command with the reviewed directory and sanitized environment. The payload stays on the restored surface context until the proxy publishes its first stream identity, is queued exactly once, and is not replayed on reconnect. Scheduling and delivery diagnostics omit command and environment content. The real loopback SSH Actions scenario records a binding, approves it through the GTK Preferences panel, proves approval itself is inert, restarts normally, and then requires the marker to be written remotely. Mosh restoration embeds the same reviewed subshell only in the quoted remote command used by both Mosh and its direct-SSH fallback, and returns to an interactive remote shell; it never launches the agent binding as a local Ghostty command. Executable builder tests cover shell and named tmux argument construction while full UDP integration remains environment-dependent.

Remote terminal EOF now emits `remote.terminal.eof` with the stable `retain_until_next_input` policy and no terminal content. The loopback SSH fixture exits one restored remote shell, requires the surface to remain visible after the EOF record, sends one key, requires only that ended surface to disappear, and then verifies the application socket remains responsive.
...
### Hermes Agent and Kimi lifecycle hooks

Hermes uses its native YAML event mapping plus the companion JSON shell-hook allowlist. Linux inserts bounded marker-owned list entries into existing event keys, retains user commands and top-level configuration, and adds exact allowlist approvals without removing unrelated records. Lifecycle events bind `hermes --resume SESSION_ID`, record `pre_llm_call`, publish response/approval attention and clear only `on_session_finalize`; turn-level `on_session_end` remains non-destructive. Kimi uses a marker-owned series of native `[[hooks]]` TOML tables under `KIMI_CODE_HOME`, `KIMI_SHARE_DIR` or the current default, preserving all user TOML outside the block. It binds `kimi --resume SESSION_ID` and covers start, prompt, notification, stop and true end. The combined Actions fixture verifies config idempotence, preserved user content, Hermes approvals, nested Hermes session identity, exact resume argv and GTK routing.
...
Each tab label publishes its stable surface UUID as a native GTK move drag. A pane-center drop resolves the current tab position and transfers or reorders it, the nearest outer quarter creates a left/right/up/down split, and a workspace sidebar-row drop targets that workspace's focused pane. Drop actions resolve ownership at delivery time and log bounded rejection reasons. `pane.list` exposes optional realized pane and tab-label bounds relative to the top-level window for pointer automation and layout triage. The Actions fixture calls the same bounded drag-to-split action directly, then verifies unique identity and restored topology; it does not depend on timing-sensitive synthetic X11 pointer motion. Surface counting covers every sibling tab rather than equating one pane with one surface. A final browser may leave a remote workspace because closing that source retires its workspace-owned SSH bridge and the browser independently rebinds to the destination route while retaining its UUID, URL and profile. The real SSH/browser fixture closes the source terminal, moves that last browser, verifies source removal and exercises the lazy destination browser. A final terminal stays in its remote workspace because its reconnect task and PTY bridge are workspace-owned; the move is rejected before mutation so the live remote process is preserved. This explicit Linux boundary closes the row without introducing a second transport-ownership model.
...
Durable review comments are stored by canonical Git repository and exposed without a running application through `cmux comments list/add/delete/consume`. Pending entries carry stable UUIDs, old/new line ranges, captured line text, a message and a ready-to-copy agent submission. Git-backed diff generation snapshots pending comments into the private viewer, which renders them at the range's ending line in either layout. A click on an old/new line opens a native-page composer; its bounded request crosses the existing browser location watcher only when the canonical page is directly inside cmux's private diff directory. Durable storage acknowledgement updates the live card, while UUID replay makes polling idempotent. The real Chromium fixture composes in the view and observes the resulting entry through the independent CLI. Consumed comments remain available through `list --all`, while delete removes them. Per-repository locking prevents lost updates across agents; storage is owner-only, atomically replaced, capped at 512 comments and 1 MiB, and pruned globally to 128 repositories or 64 MiB.

This checkpoint remains incomplete parity evidence until Actions passes.

### Manifest-aware Linux project inspector checkpoint

Current upstream commit `7d78b6e4cb0236c3b4eb354d851498b685801e00` exposes an Xcode-specific `project.open` surface with Files, Targets, Build Settings and Schemes tabs. `cmux project PATH` now preserves that inspection workflow on Linux through a private self-contained browser document placed to the right of the caller. Its Files tab supports path filtering; Targets extracts package, binary, script, module, Make and CMake target summaries; Settings identifies supported manifests and bounded scan counts; Actions discloses the same resolved `cmux.json` definitions used by the reviewed command palette.

The scan resolves one regular manifest or directory, skips symlinks and generated dependency/cache trees, follows at most twelve directory levels, and fails above 20,000 examined or 5,000 visible entries, 128 supported manifests, one MiB per manifest, four MiB aggregate manifest content or three seconds. It parses Cargo and Python TOML, Node JSON, Go module declarations and conservative Make/CMake target declarations without invoking build tools or project scripts. Generated pages retain at most 32 files or 128 MiB, use owner-only atomic writes, escape script delimiters and insert visible content through `textContent`. The browser surface retains normal automation, stable routing, focus preservation and session restoration. Mock-backed and real Chromium Actions scenarios are pending.
