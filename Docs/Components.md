...
| Terminal adapter | Ghostty configuration, input, clipboard and rendering through its C ABI; surface registry owns routing and last-known directories through teardown; events.rs bounds deferred native actions; text.rs owns non-selecting viewport capture and native buffer release | `src/ghostty` |
