...
| Browser adapter | Optional browser daemon, commands, previews and navigation; ui.rs owns GTK tab wiring and action orchestration | `src/browser.rs`, `src/browser/{ui,input,input_queue,cli,discovery,transport,frames,pixels,stream,motion,metrics}.rs` |
