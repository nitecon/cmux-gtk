...
This table supersedes the historical checkpoint notes below. Inventory refreshed from tracked, existing unique owned source through `da7b57c3` on 2026-09-06 UTC, with the obsolete split-flash harness removed in this checkpoint. Ghostty and nine Python entry symlinks are excluded from owned declaration counts. Static scans support discovery and documentation checks; they do not prove runtime correctness.
