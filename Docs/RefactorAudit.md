...
The Go test executable also compiles without running. Native PTY review identified remaining lifecycle limitations: deadline methods are no-ops and Close can wait indefinitely for a child that ignores termination. The extraction does not claim those behaviors are solved; bounded shutdown and native dimension validation remain for review.
