...
This table supersedes the historical checkpoint notes below. Inventory refreshed from `a4ab67a7` on 2026-09-05. Counts include tracked, existing unique owned source files and exclude Ghostty and Python entry symlinks (four links reuse source counted elsewhere). Static declaration counts are not a completion claim.
...
| Document every owned function | Both Python clients and six maintained Python scripts have function docstrings; earlier Rust/Go declaration passes are recorded below. | Recursive Python AST scan finds 243 undocumented declarations among 533 functions in 70 `tests` files, and 598 among 789 functions in 85 `tests_v2` files. All 22 functions in the six maintained `scripts` Python files have docstrings. Audit unsupported scenarios before documenting them. Continue semantic review of existing contracts and embedded script helpers. |
...
| Executable verification | Run 33996829835 at `ed7463c2` completed successfully, covering typed input/read, SIGINT, close selection, split validation, memory churn, clipboard, SSH and optimized benchmarks. Archived raw earlier baseline remains under Docs/Benchmarks/e08b976f. | Later reorder/target/envelope validation, fixture consolidation and transport separation still need cumulative CI. Run 33997216714 is live and 33997264871 is pending when inspected. No local tests are permitted. |
