...
This table supersedes the historical checkpoint notes below. Inventory refreshed from tracked, existing unique owned source through `da7b57c3` on 2026-09-06 UTC, with the obsolete split-flash harness removed in this checkpoint. Ghostty and two Python entry symlinks are excluded from owned declaration counts. Static scans support discovery and documentation checks; they do not prove runtime correctness.
...
Retired the unsupported terminal_input_render_report utility, its v2 entry link and the report-writer-only unit suite/CI invocation. Its sole consumer depended on absent panel snapshots/render stats/native focus APIs, so preserving a general HTML writer would add unused infrastructure. Removed the four newly orphaned snapshot/render/focus methods from each Python client. Preserve future native visual diagnostics: initial terminal and a new sibling after four right splits; three caret captures spaced 650 ms, before/200-ms-after-character/350-ms-after-Enter images, metadata and counters, privately generated bounded output. Successful report generation never proved blink or glyph correctness. Existing bounded JSON benchmark/diagnostic artifacts continue to serve implemented workflows. Hidden workflow references were included in caller checks; AST/docstring and diff validation pass, no local runtime tests.

Retired the upstream sidebar metadata, markdown-block, PR, ports, CWD/Git and split/workspace CWD-inheritance scenarios plus their now-unused sidebar_support helper/unit suite and CI invocation. GTK exposes none of their sidebar_state/report_meta/report_pr/ports/Git commands; native per-surface directory reporting tests remain. Preserve deferred contracts: metadata icon/URL/format/priority ordering with set_status alias and clear; ordered markdown blocks; PR→MR provider label and open→merged→clear; exclude external listeners then attribute the same port to a shell child and clear it on exit; directory/repository enter/leave and alias branch switching; branch change during a six-second foreground command must appear before prompt return without forced-report fallback; split/workspace inheritance from source directories including spaces. Existing tests sometimes forced PWD/Git reports, so their successful transitions did not prove automatic discovery. Current native directory tests are not proof of all inheritance or Git/port behavior. No production feature removed or deferred feature started. Caller checks include workflows; static AST/docstring and diff checks pass.

Retired two unsupported notification suites and their v2 links after comparing their inbox/read/flash fields to the actual workspace-attention API. Preserve deferred behavior: suppress insertion only with app+panel focus, retain unread when inactive, OSC99 simple and chunked title/body (no publication before final chunk), OSC777 title/body, mark read on pane/app/workspace activation while retaining entries, dismissal highlight, click-to-focus exact surface, last-focused-pane restoration, and removal on workspace closure. Updated docs/notifications.md to document real list-notifications/clear-notification commands and Linux desktop delivery; removed macOS notify/AppleScript hook instructions that cannot work here. Native bell/attention benchmark and bounded desktop-helper tests remain. Shared result_support still has the control-socket fixture as a caller. AST/docstring and diff checks pass; no local tests or deferred feature implementation.

Retired two unsupported browser shortcut harnesses and one v2 link: Cmd-bracket back/forward and Cmd-Option-arrow pane movement depended on WKWebView first-responder hooks, mutable debug shortcuts and upstream browser-pane creation. Preserve eventual native shortcut scenarios: loaded A→B navigation, back to A/forward to B with browser focus, browser-history shortcuts not changing terminal workspace selection, and terminal→loaded-browser→terminal movement while the actual browser input widget has focus. Existing GTK navigation/lifecycle and real Chromium DOM/preview coverage remains; it does not establish those exact macOS shortcut contracts. No maintained caller used either harness. AST/docstring and diff validation pass; production behavior is unchanged.

Retired the final legacy control-socket suite and its now-unused result_support record, including both v2 links. It required named upstream control-key commands and a specific terminfo/resource layout, repeatedly replaying mutations to obtain marker success. Preserve shell-resumption scenarios after interrupting sleep/Python, EOF to cat followed by a new shell command, and usable Ghostty terminfo/resource discovery with XDG defaults. Current native SIGINT, EOF/memory cleanup and platform/package checks remain; they do not establish every old environment-path convention. Python transport/client compatibility tests and the maintained prompt probe still use the clients, so those clients remain pending removal of individually orphaned methods. AST/docstring and diff validation pass; no local tests ran.

The maintained Pure prompt probe still used upstream workspace_id response fields and pasted newline rather than submitting Enter. Adapted it to GTK uuid/id fields and surface.send_key carriage return, and corrected its viewport contract (bounded visible text, not eighty scrollback lines). Cleanup now restores the original workspace even when close fails, while surfacing cleanup failure. The parser stops at earlier prompt rows so repeated ordinary prompts are not mistaken for duplicate preprompt context. Added native Actions coverage with a controlled Bash prompt: its hook records shell PIDs, proving at least three actual submissions and checking temporary-workspace cleanup/restoration. Existing mocked failure cleanup also uses current protocol fields. Syntax checks pass; runtime verification remains in CI.

Run34014097102 atb1b6e9c6 failed the new nested routing fixture before input: surface.list intentionally includes every workspace, while the fixture compared its entire result against only the three test panes. Filtered by workspace_uuid, retaining exact membership assertions. The same contract review corrected the maintained prompt probe to select the active surface belonging to its temporary workspace rather than the first globally listed surface. No production routing bug was established by this failure; later native prompt and cumulative CI evidence remain required.

Pruned Python clients by maintained callers and internal dependency closure: removed 146 unused convenience methods, the orphaned option-quoting/control-escape helpers, and unused reference-resolution adapters whose only callers were mock-only tests against upstream response shapes. Removed those two obsolete resolver tests; retained real socket-pair response validation, structured-error connection reuse, discovery/transport tests, raw CLI entry points, system queries and the maintained prompt diagnostic. Module descriptions now state their actual roles. No GTK production API was changed. This removes unsupported command catalogs rather than carrying them as nominal feature coverage. AST/docstring validation finds 150 functions in 41 root test files and 34 in five tests_v2 sources; shared script count remains 29. Runtime verification stays in Actions.
