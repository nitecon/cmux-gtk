...
This table supersedes the historical checkpoint notes below. Inventory refreshed from the working tree based on `b34e2636` on 2026-09-05. Counts include tracked, existing unique owned source files and exclude Ghostty and Python entry symlinks (twelve links reuse source counted elsewhere). Static declaration counts are not a completion claim.
