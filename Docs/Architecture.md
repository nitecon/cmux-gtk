...
The inherited `web` Next.js/React/TypeScript website is not loaded by the desktop or needed to build its binaries. It and obsolete upstream macOS artifacts are cleanup candidates; see [RefactorAudit](RefactorAudit.md) for current migration status. Keep the complete Ghostty submodule, including upstream platform directories and vendored dependencies.
