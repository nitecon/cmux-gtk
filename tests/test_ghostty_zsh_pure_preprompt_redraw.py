...
This scenario checks for stale copies of the old top line in captured output.
...
def _capture_session(
    *,
    ...
    """Capture the isolated plain or Ghostty-integrated prompt and strip ANSI controls."""
    ...
    try:
        ...
        output = capture_prompt_session([zsh_path, "-d", "-i"], env, cwd=workdir)
...
def _stale_preprompt_lines(cleaned: str, path_line: str, async_line: str) -> tuple[int, int]:
    """Count repeated path and async lines after the first async prompt, or return missing sentinels."""
...
def main() -> int:
    """Compare repeated preprompt lines between plain zsh and the Ghostty integration."""
