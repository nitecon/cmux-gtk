...
/** Parse file metadata without line anchors; only hunk content advances source line numbers. */
const parse=p=>{
  const out=[];if(!p.trim())return out;
  let f=null,old=0,neu=0,inHunk=false;
  for(const text of p.split(/\n/)){
    if(text.startsWith('diff --git ')){
      f={name:text.replace(/^diff --git a\//,'').replace(/ b\/.*$/,''),lines:[]};
      out.push(f);inHunk=false;
    }
    if(!f){f={name:'Patch',lines:[]};out.push(f)}
    let kind='ctx',o='',n='';
    const h=text.match(/^@@ -(\d+)(?:,\d+)? \+(\d+)(?:,\d+)? @@/);
    if(h){old=+h[1];neu=+h[2];kind='hunk';inHunk=true}
    else if(inHunk&&text.startsWith('+')){kind='add';n=neu++}
    else if(inHunk&&text.startsWith('-')){kind='del';o=old++}
    else if(inHunk&&text.startsWith(' ')){o=old++;n=neu++}
    f.lines.push({text,kind,o,n});
  }
  return out;
};
