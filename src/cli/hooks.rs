...
const PI_EXTENSION_TEMPLATE: &str = r#"// cmux-pi-session-extension-marker v1
// Generated by cmux. Bridges Pi's native extension lifecycle into the owning surface.
...
function send(subcommand, hookEventName, event, context) {
  if (process.env.CMUX_PI_HOOKS_DISABLED === "1" || !process.env.CMUX_SURFACE_ID) return;
...
fn setup_pi(explicit: bool) -> Result<(), CliError> {
    let provider = json_provider("pi").expect("static provider");
    if cmux_platform::paths::find_command_on_path(provider.binary).is_none() {
        if explicit {
            return Err(CliError::Command(
                "install Pi before installing its extension".into(),
            ));
        }
        println!("Skipped Pi: executable not found on PATH");
        return Ok(());
    }
    let agent_directory = std::env::var_os("PI_CODING_AGENT_DIR")
        ...
        .ok_or_else(|| CliError::Command("cannot resolve Pi agent directory".into()))?;
    let directory = agent_directory.join("extensions");
    cmux_platform::filesystem::create_private_directory(&directory)
        .map_err(|error| CliError::Command(format!("create Pi extensions directory: {error}")))?;
    let path = directory.join("cmux-session.ts");
    if path.is_symlink() {
        return Err(CliError::Command(
            "Pi extension is a symlink; configure its target explicitly".into(),
        ));
    }
    match cmux_platform::filesystem::read_text_bounded(&path, 1024 * 1024) {
        Ok(existing)
            if !existing.is_empty() && !existing.contains("cmux-pi-session-extension-marker") =>
        {
            return Err(CliError::Command(
                "Pi extension exists without the cmux ownership marker".into(),
            ));
        }
        Ok(_) => {}
        Err(error) if error.kind() == std::io::ErrorKind::NotFound => {}
        Err(error) => return Err(CliError::Command(format!("read Pi extension: {error}"))),
    }
    let binary = std::env::current_exe().map_err(|error| CliError::Command(error.to_string()))?;
    let encoded_binary = serde_json::to_string(&binary.to_string_lossy())
        .map_err(|error| CliError::Command(error.to_string()))?;
    let source = PI_EXTENSION_TEMPLATE.replace("__CMUX_BINARY__", &encoded_binary);
    cmux_platform::filesystem::atomic_write(&path, source.as_bytes())
        .and_then(|_| cmux_platform::filesystem::sync_file_and_parent(&path))
        .map_err(|error| CliError::Command(format!("save Pi extension: {error}")))?;
    println!("Installed Pi lifecycle extension in {}", path.display());
    Ok(())
}
